...
interface ErSidebarProps {
    ...
    diagramObjects: ParsedErObjects;
    onDeleteObject: () => void;
}

export const ErSidebar: React.FC<ErSidebarProps> = (props) => {
    const { 
        sidebarSize, 
        code, 
        onCodeChange, 
    ...
    const selectedItemData = useMemo(() => {
        ...
        if (selectedObject.type === 'entity') {
            return diagramObjects.entities.find(p => p.id === selectedObject.id);
        }
        if (selectedObject.type === 'relationship') {
            return diagramObjects.relationships.find(m => m.id === selectedObject.id);
        }
        return null;
    ...
    return (
        ...
        >
            ...
            >
                ...
                <CollapsibleObjectList title="Entities" objects={diagramObjects.entities} icon="square" selectedId={selectedObject?.id ?? null} onSelect={handleSelectObject} type="entity" />
                <CollapsibleObjectList title="Relationships" objects={diagramObjects.relationships} icon="link" selectedId={selectedObject?.id ?? null} onSelect={handleSelectObject} type="relationship" />
            </div>
